    ...
    <form id="contact-form" class="flex-[3_3_0]" @submit="handleSubmit">
      <div class="grid gap-4 py-10 md:grid-cols-2 gap-x-5 gap-y-6 md:gap-y-8">
        <div>
          ...
          <input v-model="support.firstName" id="firstName" class="w-full form-input" placeholder="Enter first name"
                 name="firstName" required/>
        </div>
        <div>
          <label class="font-medium input-label text-ash" for="lastName">Last name</label>
          <input v-model="support.lastName" id="lastName" class="w-full form-input" placeholder="Enter last name"
                 name="lastName" required/>
        </div>
        <div>
          <label class="font-medium input-label text-ash" for="businessEmail">Business email</label>
          <input v-model="support.email" id="businessEmail" class="w-full form-input" placeholder="Enter email address"
                 name="businessEmail"
                 required type="email"/>
        </div>
        <div>
        ...
        <div>
          ...
          <input v-model="support.country" id="country" class="w-full form-input" placeholder="Select country"
                 name="country"/>
        </div>
        <div>
          <label class="font-medium input-label text-ash" for="countrySize">Company size</label>
          <input v-model="support.companySize" id="countrySize" class="w-full form-input"
                 placeholder="Select company size" name="companySize"/>
        </div>
        ...
        <div class="flex justify-center my-3 md:col-span-2">
          <x-button type="submit" color="primary" :class="['w-1/2 rounded-3xl', loading && 'animate-pulse']">
